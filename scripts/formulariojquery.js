...
function validarEmail(input) {
    ...
    if (validarInput(input)) {
        if (regexEmail.test(input.val())) {
            ...
            input.next().show();
            input.next().text('Correo Inválido!');
            input.removeClass('is-valid');
            input.addClass('is-invalid');
            return false;
        }
...
function validarContrasenaSegura(input) {
    ...
    if (regexContrasenaSegura.test(input.val())) {
        input.removeClass('is-invalid');
        input.addClass('is-valid');
        return true;
    } else {
        input.next().show();
        input.next().text('Contraseña no segura!');
        input.removeClass('is-valid');
        input.addClass('is-invalid');
        return false;
    }
...
function limpiarFormulario() {
    const formulario = $('#formularioRegistro');
    formulario.find('input, select').val('');
    formulario.find('input, select').removeClass('is-valid is-invalid');
};
...
function validarRutInput(input) {
    if (validarInput(input)) { // Primero verifica que no esté vacío
        ...
        if (RutValidator.validar(rutValor)) {
            ...
            input.next().show();
            input.next().text('RUT Inválido! Use el formato correcto.');
            input.removeClass('is-valid');
            input.addClass('is-invalid');
            return false;
        }
